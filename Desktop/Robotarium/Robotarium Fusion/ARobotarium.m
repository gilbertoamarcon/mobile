classdef ARobotarium < handle
    ...
    methods         
        function this = ARobotarium(number_of_agents, save_data, show_figure, initial_poses)
            global TESTBED_SIZE
            global TIME_FACTOR
            this.time_step = TIME_FACTOR;
            
            this.boundaries = [-TESTBED_SIZE(1)/2, TESTBED_SIZE(1)/2, -TESTBED_SIZE(2)/2, TESTBED_SIZE(2)/2];    
            this.boundary_points = {[-TESTBED_SIZE(1)/2, TESTBED_SIZE(1)/2, TESTBED_SIZE(1)/2, -TESTBED_SIZE(1)/2]*0.95, ... 
                [-TESTBED_SIZE(2)/2, -TESTBED_SIZE(2)/2, TESTBED_SIZE(2)/2, TESTBED_SIZE(2)/2]*0.95};
    ...
    methods (Access = protected)
        ...
        function initialize_visualization(this)
            ...
            xlim([-TESTBED_SIZE(1)/2, TESTBED_SIZE(1)/2]); ylim([-TESTBED_SIZE(2)/2, TESTBED_SIZE(2)/2]);  % static limits
